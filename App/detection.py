...
def get_labels_dics():
    ...
    classes_names_dic = {int(key): value for key, value in classes_names_dic['names'].items()}
...
def create_colors_info(team1_name, team1_p_color, team2_name, team2_p_color):
    team1_p_color_rgb = ImageColor.getcolor(team1_p_color, "RGB")
    team2_p_color_rgb = ImageColor.getcolor(team2_p_color, "RGB")

    colors_dic = {
        team1_name:[team1_p_color_rgb],
        team2_name:[team2_p_color_rgb]
    }
    ...
    for frame_nbr in range(1, tot_nbr_frames+1):
        ...
        if success:
            ...
            if 'homog' in locals():
                ...
                bboxes_p_c_1 = bboxes_p_c[[i==1 for i in labels_p],:]                       # Get bounding boxes information (x,y,w,h) of detected players (label 0)

                # Get coordinates of detected players on frame (x_cencter, y_center+h/2)
                detected_ppos_src_pts = bboxes_p_c_1[:,:2]  + np.array([[0]*bboxes_p_c_1.shape[0], bboxes_p_c_1[:,3]/2]).transpose()
                ...
                for pt in detected_ppos_src_pts:                                            # Loop over players frame coordiantes
                    pt = np.append(np.array(pt), np.array([1]), axis=0)                     # Covert to homogeneous coordinates
